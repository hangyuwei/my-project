...
export function adaptCartGroupData(real = {}) {
  if (real && real.data && real.data.storeGoods) return real;

  const source = ensureObject(real);
  const items = ensureArray(source.items || source.goods || source.cartItems || source.data?.items || source);
  if (!items.length) {
    return {
      data: {
        isNotEmpty: false,
  ...
  const goodsPromotionList = items.map((item) => {
    const sku = ensureObject(item);
    const quantity = toInt(pickFirst(sku.quantity, 1), 1);
    const price = toInt(pickFirst(sku.price, sku.payPrice, 0), 0);
    const originPrice = toInt(pickFirst(sku.originPrice, sku.linePrice, price), price);
    return {
      uid: toString(pickFirst(sku.uid, 'local')),
      saasId: toString(pickFirst(sku.saasId, 'local')),
      storeId: toString(pickFirst(sku.storeId, 'local')),
      spuId: toString(pickFirst(sku.spuId, sku.productId, '')),
      skuId: toString(pickFirst(sku.skuId, sku.variantId, sku.productId, '')),
      isSelected: pickFirst(sku.isSelected, 1),
      thumb: pickFirst(sku.thumb, sku.image, sku.skuImage, ''),
      title: pickFirst(sku.title, sku.name, sku.goodsName, ''),
      primaryImage: pickFirst(sku.primaryImage, sku.image, sku.thumb, ''),
      quantity,
      stockStatus: pickFirst(sku.stockStatus, true),
      stockQuantity: toInt(pickFirst(sku.stockQuantity, sku.stock, 999), 999),
      price: toString(price),
      originPrice: toString(originPrice),
      tagPrice: null,
      titlePrefixTags: null,
      roomId: pickFirst(sku.roomId, null),
      specInfo: ensureArray(sku.specInfo || sku.specs),
      joinCartTime: pickFirst(sku.joinCartTime, new Date().toISOString()),
  ...
  const storeId = goodsPromotionList[0]?.storeId || 'local';
  const storeName = pickFirst(source.storeName, source.store?.name, 'Local Store');
      ...
      storeGoods: [
        {
          ...
          storeName,
          storeStatus: 1,
          totalDiscountSalePrice: toString(totalAmount),
          promotionGoodsList: [
            {
              title: null,
              promotionCode: 'EMPTY_PROMOTION',
