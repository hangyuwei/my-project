...
const mapOrderItem = (item = {}, baseItem = {}) => {
  ...
  return {
    ...
    roomId: pickFirst(source.roomId, baseItem.roomId, null),
...
export function adaptOrders(real = {}, params = {}) {
  ...
  const baseOrder = baseResp.data.orders[0] || {};
  const baseItem = ensureArray(baseOrder.orderItemVOs)[0] || {};

  const orders = list.map((order) => {
    const info = ensureObject(order);
    const items = ensureArray(info.orderItemVOs || info.items || info.goods).map((item) =>
    ...
    return {
      ...
      storeId: pickFirst(info.storeId, baseOrder.storeId),
      storeName: pickFirst(info.storeName, baseOrder.storeName),
      ...
      freightFee: toString(pickFirst(info.freightFee, baseOrder.freightFee, '0')),
      discountAmount: toString(pickFirst(info.discountAmount, baseOrder.discountAmount, '0')),
  ...
  const pageNum = toInt(pickFirst(params?.parameter?.pageNum, source.pageNum, 1), 1);
  const pageSize = toInt(pickFirst(params?.parameter?.pageSize, source.pageSize, orders.length), orders.length);
  const totalCount = toInt(pickFirst(source.totalCount, orders.length), orders.length);
...
export function adaptOrdersCount(real = {}) {
  ...
  return {
    data: list.map((item) => ({
      tabType: pickFirst(item.tabType, item.status, 0),
      orderNum: pickFirst(item.orderNum, item.count, 0),
    })),
    code: 'Success',
    msg: null,
    requestId: mockReqId(),
