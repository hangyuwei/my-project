...
const mapOrderDetailItem = (item = {}, baseItem = {}) => {
  const source = ensureObject(item);
  return {
    ...baseItem,
    id: pickFirst(source.id, baseItem.id, ''),
    orderNo: pickFirst(source.orderNo, baseItem.orderNo, null),
    spuId: toString(pickFirst(source.spuId, source.productId, baseItem.spuId, '')),
    skuId: toString(pickFirst(source.skuId, source.variantId, baseItem.skuId, '')),
    goodsName: pickFirst(source.goodsName, source.name, baseItem.goodsName, ''),
    goodsPictureUrl: pickFirst(source.goodsPictureUrl, source.image, baseItem.goodsPictureUrl, ''),
    originPrice: toString(pickFirst(source.originPrice, source.linePrice, baseItem.originPrice, '0')),
    actualPrice: toString(pickFirst(source.actualPrice, source.price, baseItem.actualPrice, '0')),
    specifications: ensureArray(source.specifications || source.specInfo || baseItem.specifications),
    buyQuantity: toInt(pickFirst(source.buyQuantity, source.quantity, 1), 1),
    itemTotalAmount: toString(pickFirst(source.itemTotalAmount, source.totalAmount, baseItem.itemTotalAmount, '0')),
    itemDiscountAmount: toString(pickFirst(source.itemDiscountAmount, baseItem.itemDiscountAmount, '0')),
    itemPaymentAmount: toString(pickFirst(source.itemPaymentAmount, baseItem.itemPaymentAmount, '0')),
    goodsPaymentPrice: toString(pickFirst(source.goodsPaymentPrice, baseItem.goodsPaymentPrice, '0')),
    tagPrice: pickFirst(source.tagPrice, baseItem.tagPrice, null),
    tagText: pickFirst(source.tagText, baseItem.tagText, null),
    outCode: pickFirst(source.outCode, baseItem.outCode, null),
    labelVOs: pickFirst(source.labelVOs, baseItem.labelVOs, null),
    buttonVOs: pickFirst(source.buttonVOs, baseItem.buttonVOs, null),
  };
};
...
export function adaptOrderDetail(real = {}, params = {}) {
  ...
  const baseData = ensureObject(base.data);
  const baseItem = ensureArray(baseData.orderItemVOs)[0] || {};
  const items = ensureArray(source.orderItemVOs || source.items || source.goods).map((item) =>
    mapOrderDetailItem(item, baseItem),
  );
  ...
  return {
    ...
    data: {
      ...baseData,
      orderNo: toString(pickFirst(source.orderNo, baseData.orderNo)),
      orderStatus: pickFirst(source.orderStatus, source.status, baseData.orderStatus),
      orderStatusName: pickFirst(source.orderStatusName, source.statusName, baseData.orderStatusName),
      totalAmount: toString(pickFirst(source.totalAmount, source.amount, baseData.totalAmount, '0')),
      goodsAmount: toString(pickFirst(source.goodsAmount, baseData.goodsAmount, '0')),
      paymentAmount: toString(pickFirst(source.paymentAmount, source.payAmount, baseData.paymentAmount, '0')),
      freightFee: toString(pickFirst(source.freightFee, baseData.freightFee, '0')),
      discountAmount: toString(pickFirst(source.discountAmount, baseData.discountAmount, '0')),
      createTime: pickFirst(source.createTime, source.createdAt, baseData.createTime),
      orderItemVOs: items.length ? items : baseData.orderItemVOs,
    },
  };
}

export function adaptBusinessTime(real = {}) {
  if (real && real.data && real.data.businessTime) return real;
  const source = ensureObject(real);
  const base = genBusinessTime();
  return {
    ...base,
    data: {
      ...base.data,
      businessTime: pickFirst(source.businessTime, source.timeList, base.data.businessTime),
      telphone: pickFirst(source.telphone, source.phone, base.data.telphone),
      saasId: pickFirst(source.saasId, base.data.saasId),
    },
  };
}
