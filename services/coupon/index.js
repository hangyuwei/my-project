...
import { adaptCouponDetail, adaptCouponList } from '../../model/coupon';
...
export function fetchCouponList(status = 'default') {
  if (shouldMock('coupon.fetchCouponList')) {
    return mockFetchCoupon(status);
  }
  return callCloudFunction('getCouponList', { status }).then((real) => adaptCouponList(real));
}
...
export function fetchCouponDetail(id, status = 'default') {
  if (shouldMock('coupon.fetchCouponDetail')) {
    return mockFetchCouponDetail(id, status);
  }
  return callCloudFunction('getCouponDetail', { id, status }).then((real) =>
    adaptCouponDetail(real),
  );
}
