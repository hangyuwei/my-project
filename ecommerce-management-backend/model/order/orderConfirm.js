...
export function adaptSettleDetail(real = {}) {
  if (real && real.data && real.code) return real;
  const source = ensureObject(real);
  const stripUndefinedRoomId = (resp) => {
    const list = ensureArray(resp?.data?.storeGoodsList);
    list.forEach((store) => {
      const skus = ensureArray(store?.skuDetailVos);
      skus.forEach((sku) => {
        if (sku && Object.prototype.hasOwnProperty.call(sku, 'roomId') && sku.roomId === undefined) {
          delete sku.roomId;
        }
      });
    });
  };
  if (source.goodsRequestList || source.couponList || source.userAddressReq) {
    const resp = genSettleDetail({
      goodsRequestList: ensureArray(source.goodsRequestList),
      couponList: ensureArray(source.couponList),
      userAddressReq: source.userAddressReq || null,
    });
    stripUndefinedRoomId(resp);
    return resp;
  }
  const resp = genSettleDetail({
    goodsRequestList: ensureArray(source.items || source.goods || []),
    couponList: ensureArray(source.coupons || []),
    userAddressReq: source.userAddress || null,
  });
  stripUndefinedRoomId(resp);
  return resp;
}

export function adaptCommitPay(real = {}) {
  if (real && real.data && real.code) return real;
  const source = ensureObject(real);
  const isSuccess = pickFirst(source.isSuccess, source.success, true);
  return {
    data: {
      isSuccess,
      tradeNo: toString(pickFirst(source.tradeNo, source.orderNo, '')),
      payInfo: pickFirst(source.payInfo, '{}'),
      code: source.code ?? null,
      transactionId: toString(pickFirst(source.transactionId, '')),
      msg: source.msg ?? null,
      interactId: source.interactId ?? '15145',
      channel: pickFirst(source.channel, 'wechat'),
      limitGoodsList: source.limitGoodsList ?? null,
    },
    code: pickFirst(source.code, 'Success'),
    msg: source.msg ?? null,
    requestId: mockReqId(),
    clientIp: mockIp(),
    rt: 0,
